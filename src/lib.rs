...
pub enum Error {
    ...
    ProposalExpired = 35,
    TrancheNotFound = 36,
    TrancheAlreadyClaimed = 37,
    MemberDefaulted = 38,
    TrancheNotUnlocked = 39,
    TrancheClawbackFailed = 40,
}
...
const LENDING_MARKET_VOTING_PERIOD: u64 = 86400; // 24 hours
const LENDING_MARKET_EMERGENCY_PERIOD: u64 = 21600; // 6 hours
const BASE_INTEREST_RATE_BPS: u32 = 500; // 5% base annual interest rate
const MIN_LENDING_AMOUNT: i128 = 100_000_000; // Minimum 10 tokens
const LIQUIDITY_PROVIDER_YIELD_BPS: u32 = 200; // 2% annual yield
// Liquidity buffer constants
const LIQUIDITY_BUFFER_ADVANCE_PERIOD: u64 = 172800; // 48 hours
const LIQUIDITY_BUFFER_MIN_REPUTATION: u32 = 10000; // 100% reputation required
const LIQUIDITY_BUFFER_MAX_ADVANCE_BPS: u32 = 10000; // 100% of contribution
const LIQUIDITY_BUFFER_PLATFORM_FEE_ALLOCATION: u32 = 2000; // 20% of platform fees
const LIQUIDITY_BUFFER_MIN_RESERVE: i128 = 1_000_000_000; // Minimum 100 tokens
const LIQUIDITY_BUFFER_MAX_RESERVE: i128 = 10_000_000_000; // Maximum 10,000 tokens
const LIQUIDITY_BUFFER_ADVANCE_FEE_BPS: u32 = 50; // 0.5% fee
const LIQUIDITY_BUFFER_GRACE_PERIOD: u64 = 86400; // 24 hours grace period
const LIQUIDITY_BUFFER_MAX_ADVANCES_PER_ROUND: u32 = 3; // Maximum advances per round
...
// --- MISSING TYPES FROM LATER FEATURE BRANCHES ---
...
pub struct LiquidityBufferConfig {
    pub is_enabled: bool,
    pub advance_period: u64,
    pub min_reputation: u32,
    pub max_advance_bps: u32,
    pub platform_fee_allocation: u32,
    pub min_reserve: i128,
    pub max_reserve: i128,
    pub advance_fee_bps: u32,
    pub grace_period: u64,
    pub max_advances_per_round: u32,
}
...
pub struct LiquidityBufferStats {
    ...
    pub buffer_utilization_rate: u32,
    pub last_updated: u64,
}
...
pub trait SoroSusuTrait {
    ...
    fn configure_stealth_mode(env: Env, caller: Address, circle_id: u64, enabled: bool, seed: Option<u64>);
    fn reveal_next_winner(env: Env, circle_id: u64) -> Option<Address>;
    fn is_stealth_mode_enabled(env: Env, circle_id: u64) -> bool;
...
impl SoroSusuTrait for SoroSusu {
    ...
    fn finalize_round(env: Env, caller: Address, circle_id: u64) {
        ...
        if stealth_mode::is_stealth_enabled(&env, circle_id) {
            let hidden_winner_index = stealth_mode::prepare_next_winner(&env, circle_id, circle.member_count);
            circle.current_recipient_index = hidden_winner_index;
    ...
    fn configure_stealth_mode(env: Env, caller: Address, circle_id: u64, enabled: bool, seed: Option<u64>) {
        ...
        if enabled {
            let chosen_seed = seed.unwrap_or_else(|| stealth_mode::generate_random_seed(&env));
            stealth_mode::toggle_stealth_mode(&env, circle_id, true, chosen_seed);
        } else {
            stealth_mode::toggle_stealth_mode(&env, circle_id, false, 0);
        }
    }
    ...
    fn reveal_next_winner(env: Env, circle_id: u64) -> Option<Address> {
        ...
        if let Some(existing_recipient) = circle.current_pot_recipient.clone() {
            return Some(existing_recipient);
        }

        if !stealth_mode::is_stealth_enabled(&env, circle_id) {
            let recipient = get_member_address_by_index(&circle, circle.current_recipient_index);
            circle.current_pot_recipient = Some(recipient.clone());
            env.storage().instance().set(&DataKey::Circle(circle_id), &circle);
            return Some(recipient);
        }

        let winner_index = stealth_mode::reveal_winner(&env, circle_id)?;
        let recipient = get_member_address_by_index(&circle, winner_index);
        circle.current_pot_recipient = Some(recipient.clone());
        env.storage().instance().set(&DataKey::Circle(circle_id), &circle);

        env.events().publish(
            (symbol_short!("REVEALWIN"), circle_id),
            recipient.clone(),
        );
    ...
    fn is_stealth_mode_enabled(env: Env, circle_id: u64) -> bool {
        stealth_mode::is_stealth_enabled(&env, circle_id)
    }
    ...
    fn claim_pot(env: Env, user: Address, circle_id: u64) {
        ...
        let recipient = if let Some(existing) = circle.current_pot_recipient.clone() {
            existing
        } else {
            let revealed = Self::reveal_next_winner(env.clone(), circle_id)
                .unwrap_or_else(|| panic!("No recipient set"));
            circle.current_pot_recipient = Some(revealed.clone());
            revealed
        };
